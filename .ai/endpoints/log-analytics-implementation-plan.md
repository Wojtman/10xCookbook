...
## 1. Endpoint Overview
- Accepts analytics events from both anonymous sessions and authenticated users via `POST /analytics/events`.
- Validates `session_id` and `event_type` before inserting into Supabase `analytics_events`.
...
- Headers:
  ...
  - `Authorization: Bearer <token>` (optional; resolved through `locals.supabase.auth.getUser()`)
- Body Parameters (JSON shaped as `LogAnalyticsEventCommand`):
  - **Required**
    - `session_id`: trimmed string, 1–255 characters; required even when a Supabase user is present to correlate anonymous flows.
...
- **400 Bad Request** — invalid JSON body, missing required fields, disallowed `event_type`, malformed `session_id`, or oversized `event_data`; return `ErrorResponseDTO` including `fields` from Zod.
...
3. Acquire Supabase client from `locals.supabase` and call `auth.getUser()`:
   ...
   - Treat absence of a user (no auth header) as acceptable for anonymous flows.
   - If `getUser()` yields an authentication error (other than missing session), return 401.
...
6. Call `logAnalyticsEvent({ supabase: locals.supabase, userId: user?.id ?? null, command: validatedData })`.
...
- Enforce strict validation on `session_id` and `event_type` to prevent injection of arbitrary values or table pollution.
...
5. **Logging & Monitoring:** Standardize error logs with namespace `[Analytics#POST]` including `requestId`, request metadata (session_id/event_type) when safe, and use structured logging if available.
