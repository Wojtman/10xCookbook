...
- Provide a `POST /images/upload` API for authenticated users to upload recipe images.
...
- Request Body:
  - Multipart form-data payload containing the image file.
...
- Error responses:
  - 400 Bad Request — missing file, invalid dimensions after processing, failed validation.
...
- Authentication:
  - Use Supabase auth via `context.locals.supabase`; requests without an authenticated user are rejected.
- Input sanitization:
...
- Rate limiting:
  - Enforce 20 uploads/hour per user; return 429 on violation.
  - Consider storing counts in Supabase table with composite key `(user_id, hour_window)`.
- CORS & CSRF:
