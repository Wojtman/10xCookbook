...
const JsonSchemaDefinitionSchema = z
  .object({
    ...
    schema: z.any().refine((val) => typeof val === "object" && val !== null, "schema.schema must be a JSON object"),
...
export const OpenRouterChatRequestSchema = z
  .object({
    ...
    idempotency_key: z.string().trim().min(1, "idempotency_key must not be empty").max(255).optional(),
  })
  .strict();

export type OpenRouterChatRequestInput = z.infer<typeof OpenRouterChatRequestSchema>;
