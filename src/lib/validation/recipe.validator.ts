  ...
  .object({
    ...
    limit: z.coerce
      ...
      .max(
        ...
        `Limit must not exceed ${VALIDATION_CONSTANTS.PAGINATION.MAX_LIMIT}`
      )
      .optional()
      .default(VALIDATION_CONSTANTS.PAGINATION.DEFAULT_LIMIT),

    sort: z
      .enum(["display_order", "created_at", "updated_at", "title", "prep_time_minutes"])
      .optional()
    ...
    search: z.string().trim().min(1, "Search query must not be empty").optional(),

    prep_time_min: z.coerce.number().int().min(0, "Minimum prep time must be non-negative").optional(),

    prep_time_max: z.coerce.number().int().min(0, "Maximum prep time must be non-negative").optional(),
  })
  .refine(
    (data) => {
...
export const RecipeIngredientInputSchema = z.object({
  ...
  quantity: z.string().trim().max(100, "Quantity must not exceed 100 characters").nullable().optional(),

  notes: z.string().trim().max(500, "Ingredient notes must not exceed 500 characters").nullable().optional(),

  ingredient_id: z.string().uuid("Invalid ingredient ID format").nullable().optional(),
});
...
export const CreateRecipeSchema = z
  .object({
    ...
    preparation_description: z
      ...
      .trim()
      .max(
        VALIDATION_CONSTANTS.RECIPE.MAX_DESCRIPTION_LENGTH,
        `Preparation description must not exceed ${VALIDATION_CONSTANTS.RECIPE.MAX_DESCRIPTION_LENGTH} characters`
      )
      .optional()
      .default(""),
    ...
    image_alt_text: z.string().trim().max(500, "Image alt text must not exceed 500 characters").nullable().optional(),
    ...
    ingredients: z
      ...
      .max(
        VALIDATION_CONSTANTS.RECIPE.MAX_INGREDIENTS,
        `Recipe must not exceed ${VALIDATION_CONSTANTS.RECIPE.MAX_INGREDIENTS} ingredients`
      ),

    tag_ids: z.array(z.string().uuid("Invalid tag ID format")).optional().default([]),
  })
  .refine(
    (data) => {
 ...
 */
export const ReorderRecipesSchema = z
  .object({
    recipes: z.array(RecipeReorderItemSchema).min(1, "At least one recipe must be provided for reordering"),
  })
  .refine(
    (data) => {
