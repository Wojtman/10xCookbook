...
export class SessionService {
   ...
   */
  async createAnonymousSession(options: CreateAnonymousSessionOptions): Promise<SessionResponseDTO> {
    ...
    const expiresAt = new Date(
      Date.now() + VALIDATION_CONSTANTS.SESSION.ANONYMOUS_SESSION_TTL_HOURS * 60 * 60 * 1000
    ).toISOString();
    ...
    if (error || !data) {
      ...
      throw new SessionServiceError(`Failed to persist anonymous session: ${error?.message ?? "Unknown error"}`);
    }
  ...
  async migrateAnonymousSession(options: MigrateAnonymousSessionOptions): Promise<MigrationResponseDTO> {
    const { sessionToken, targetCookbookId, userId } = options;

    const sessionRow = await this.fetchActiveAnonymousSession(sessionToken);
    const destinationCookbookId = await this.resolveTargetCookbookId(userId, targetCookbookId ?? null);
    ...
    try {
      ...
      if (recipeInsertError) {
        throw new SessionServiceError(`Failed to insert migrated recipes: ${recipeInsertError.message}`);
      }

      insertedRecipeIds.push(...(insertedRecipes?.map((recipe) => recipe.id) ?? recipeIds));

      if (ingredientRows.length > 0) {
        const { error: ingredientInsertError } = await this.supabase.from("recipe_ingredients").insert(ingredientRows);

        if (ingredientInsertError) {
          throw new SessionServiceError(`Failed to insert migrated ingredients: ${ingredientInsertError.message}`);
        }
      }

      if (tagRows.length > 0) {
        const { error: tagInsertError } = await this.supabase.from("recipe_tags").insert(tagRows);

        if (tagInsertError) {
          throw new SessionServiceError(`Failed to insert migrated recipe tags: ${tagInsertError.message}`);
        }
      }
    } catch (error) {
    ...
    if (sessionUpdateError) {
      await this.rollbackInsertedRecipes(insertedRecipeIds);
      throw new SessionServiceError(`Failed to update anonymous session status: ${sessionUpdateError.message}`);
    }
  ...
  private async logSessionCreationAnalytics({
    ...
    try {
      ...
      if (error instanceof AnalyticsServiceError) {
        console.error("Failed to log anonymous_session_request analytics event for rate limiting", error);
      } else {
        console.error("Unexpected error while logging anonymous_session_request analytics event", error);
      }
    }
  }
  ...
  private async resolveTargetCookbookId(userId: string, requestedCookbookId?: string | null): Promise<string> {
    if (requestedCookbookId) {
      ...
      if (error) {
        throw new SessionServiceError(`Failed to verify target cookbook ownership: ${error.message}`);
      }
    ...
    if (fallbackError) {
      throw new SessionServiceError(`Failed to resolve fallback cookbook: ${fallbackError.message}`);
    }
  ...
  private async rollbackInsertedRecipes(recipeIds: string[]): Promise<void> {
    ...
    const { error: tagDeleteError } = await this.supabase.from("recipe_tags").delete().in("recipe_id", uniqueIds);
    ...
    if (ingredientDeleteError) {
      console.error("Failed to rollback recipe ingredients after migration failure", ingredientDeleteError);
    }

    const { error: recipeDeleteError } = await this.supabase.from("recipes").delete().in("id", uniqueIds);
  ...
  private async cleanupAnonymousDrafts(recipeIds: string[]): Promise<void> {
    ...
    if (ingredientDeleteError) {
      console.error("Failed to cleanup anonymous recipe ingredients after migration", ingredientDeleteError);
    }
    ...
    if (tagDeleteError) {
      console.error("Failed to cleanup anonymous recipe tags after migration", tagDeleteError);
    }

    const { error: recipeDeleteError } = await this.supabase.from("anonymous_recipes").delete().in("id", uniqueIds);
