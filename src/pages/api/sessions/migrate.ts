...
export const POST: APIRoute = async ({ request, locals }) => {
  ...
  try {
    ...
    return new Response(
      JSON.stringify(buildErrorResponse('invalid_json', 'Invalid JSON payload')),
      {
        status: 400,
        headers: { 'Content-Type': 'application/json' },
      },
  ...
  try {
    ...
    if (error instanceof SessionNotFoundError) {
      return new Response(
        JSON.stringify(
          buildErrorResponse('session_not_found', 'Anonymous session not found', ['session_id']),
        ),
        {
          status: 404,
          headers: { 'Content-Type': 'application/json' },
    ...
    if (error instanceof SessionExpiredError) {
      return new Response(
        JSON.stringify(
          buildErrorResponse(
            'session_expired',
            'Anonymous session has expired. Please request a new session token.',
            ['session_id'],
          ),
        ),
        {
    ...
    if (error instanceof SessionAlreadyMigratedError) {
      return new Response(
        JSON.stringify(
          buildErrorResponse(
            'already_migrated',
            'This anonymous session has already been migrated.',
            ['session_id'],
          ),
        ),
        {
    ...
    if (error instanceof AnonymousRecipesNotFoundError) {
      return new Response(
        JSON.stringify(
          buildErrorResponse(
            'session_empty',
            'No anonymous recipes found to migrate for this session.',
          ),
        ),
        {
          status: 404,
    ...
    if (error instanceof CookbookOwnershipError) {
      return new Response(
        JSON.stringify(
          buildErrorResponse(
            'cookbook_not_found',
            'Target cookbook was not found or you do not have access to it.',
            ['target_cookbook_id'],
          ),
        ),
        {
    ...
    return new Response(
      JSON.stringify(
        buildErrorResponse(
          'internal_error',
          'An unexpected error occurred while migrating anonymous recipes.',
        ),
      ),
      {
        status: 500,
