...
import { deriveClientFingerprint } from "../../../lib/validation/session.validator";
import { SessionService, SessionServiceError } from "../../../lib/services/session.service";
import { RateLimitExceededError, RateLimitServiceError } from "../../../lib/services/rateLimit.service";
...
export const POST: APIRoute = async ({ request, locals }) => {
  ...
  try {
    ...
    if (error instanceof RateLimitExceededError) {
      return new Response(
        JSON.stringify(
          buildErrorResponse("too_many_requests", "Anonymous session request limit exceeded. Please try again later.")
        ),
        {
          status: 429,
          headers: {
    ...
    if (error instanceof RateLimitServiceError) {
      ...
      return new Response(
        JSON.stringify(
          buildErrorResponse("rate_limit_error", "Unable to evaluate anonymous session rate limit. Please try again.")
        ),
        {
          status: 500,
    ...
    return new Response(
      JSON.stringify(
        buildErrorResponse("internal_error", "An unexpected error occurred while creating an anonymous session.")
      ),
      {
        status: 500,
        headers: { "Content-Type": "application/json" },
