...
export const POST: APIRoute = async ({ request, locals }) => {
  ...
  try {
    ...
    return createJsonResponse(
      buildErrorResponse("invalid_multipart", "Request must be multipart/form-data with a valid image file.", ["file"]),
      400
    );
  }
  ...
  if (!filePart || typeof (filePart as File).arrayBuffer !== "function") {
    return createJsonResponse(buildErrorResponse("file_missing", "Image file is required.", ["file"]), 400);
  }
  ...
  if (!parsedSession.success) {
    return createJsonResponse(buildErrorResponse("validation_error", parsedSession.error, ["session_id"]), 400);
  }
    ...
    return createJsonResponse(
      {
        ...buildErrorResponse(
          errorCode,
          status === 401 ? "Authentication required to perform this action." : "Unable to verify authentication status."
        ),
  ...
  if (!user && !parsedSession.data) {
    return createJsonResponse(
      buildErrorResponse("validation_error", "session_id is required for anonymous uploads.", ["session_id"]),
      400
    );
  }

  const analyticsSessionId = parsedSession.data ?? user?.id ?? `req_${requestId}`;
...
function handleUploadError(error: unknown, requestId: string): Response {
  if (error instanceof ImageUploadValidationError) {
    return createJsonResponse(buildErrorResponse(error.code, error.message, error.fields), error.status);
  }
  ...
  return createJsonResponse(
    {
      ...buildErrorResponse("internal_server_error", "An unexpected error occurred while uploading the image."),
      request_id: requestId,
    },
    500
  );
