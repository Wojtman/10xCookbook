...
interface ApiErrorBody extends ReturnType<typeof buildErrorResponse> {
  request_id?: string;
...
export const POST: APIRoute = async ({ request }) => {
  ...
  try {
    ...
    if (error instanceof ZodError) {
      ...
      return createErrorResponse(400, "validation_error", "OpenRouter chat request failed validation.", {
        requestId,
        fields,
      });
    }

    return createErrorResponse(400, "validation_error", "OpenRouter chat request failed validation.", { requestId });
  }
  ...
  try {
    ...
    return createErrorResponse(500, "openrouter_configuration_error", "OpenRouter service is not available.", {
      requestId,
    });
  }

  const { schema, timeout_ms: timeoutMs, idempotency_key: idempotencyKey, ...rest } = payload;
...
function toServiceErrorResponse(error: OpenRouterServiceError, requestId: string): Response {
  const { details } = error;
