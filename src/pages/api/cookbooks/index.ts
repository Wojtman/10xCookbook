...
import { CookbookListQuerySchema, CreateCookbookSchema } from "../../../lib/validation/cookbook.validator";
...
export const GET: APIRoute = async ({ url, locals }) => {
  try {
    ...
    return new Response(
      JSON.stringify(buildErrorResponse("internal_error", "An unexpected error occurred while listing cookbooks")),
      {
        status: 500,
        headers: { "Content-Type": "application/json" },
      }
...
export const POST: APIRoute = async ({ request, locals }) => {
  try {
    ...
    try {
      ...
      return new Response(JSON.stringify(newCookbook), {
        status: 201,
        headers: { "Content-Type": "application/json" },
      });
    } catch (error) {
    ...
    return new Response(
      JSON.stringify(buildErrorResponse("internal_error", "An unexpected error occurred while creating the cookbook")),
      {
        status: 500,
        headers: { "Content-Type": "application/json" },
      }
