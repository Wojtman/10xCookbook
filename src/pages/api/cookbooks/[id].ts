...
import { UUIDParamSchema, UpdateCookbookSchema } from "../../../lib/validation/cookbook.validator";
  ...
  try {
    ...
    try {
      ...
      if (error instanceof ZodError) {
        return new Response(JSON.stringify(buildErrorResponse("invalid_id", "Invalid cookbook ID format", ["id"])), {
          status: 400,
    ...
    return new Response(
      JSON.stringify(
        buildErrorResponse("internal_error", "An unexpected error occurred while retrieving the cookbook")
      ),
      {
        status: 500,
        headers: { "Content-Type": "application/json" },
...
export const PATCH: APIRoute = async ({ params, request, locals }) => {
  try {
    ...
    try {
      const updatedCookbook = await cookbookService.updateCookbook(cookbookId!, user.id, validatedCommand);
      ...
      return new Response(JSON.stringify(updatedCookbook), {
        status: 200,
        headers: { "Content-Type": "application/json" },
      });
    } catch (error) {
      ...
      if (error instanceof Error) {
        if (error.message === "DUPLICATE_TITLE") {
          return new Response(
            JSON.stringify(
              buildErrorResponse("duplicate_title", "A cookbook with this title already exists", ["title"])
            ),
            {
              status: 409,
              headers: { "Content-Type": "application/json" },
    ...
    return new Response(
      JSON.stringify(buildErrorResponse("internal_error", "An unexpected error occurred while updating the cookbook")),
      {
        status: 500,
        headers: { "Content-Type": "application/json" },
      }
...
export const DELETE: APIRoute = async ({ params, locals }) => {
  try {
    ...
    return new Response(
      JSON.stringify(buildErrorResponse("internal_error", "An unexpected error occurred while deleting the cookbook")),
      {
        status: 500,
        headers: { "Content-Type": "application/json" },
      }
